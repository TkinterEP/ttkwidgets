...
Edited by rdbende translations, and reliefs
...
EN = {"Red": "Red", "Green": "Green", "Blue": "Blue",
      ...
      "Cancel": "Cancel", "Color Chooser": "Color Chooser",
      "Alpha": "Alpha"}
...
DE = {"Red": "Rot", "Green": "Grün", "Blue": "Blau",
      ...
      "Cancel": "Abbrechen", "Color Chooser": "Farbwähler",
      "Alpha": "Alpha"}

HU = {"Red": "Vörös", "Green": "Zöld", "Blue": "Kék",
      "Hue": "Árnyalat", "Saturation": "Telítettség", "Value": "Érték",
      "Cancel": "Mégse", "Color Chooser": "Színválasztó",
      "Alpha": "Alfa"}
...
class ColorPicker(tk.Toplevel):
    ...
    def __init__(self, master=None, title=None, color=None, alpha=False):
        """
        ...
        style.map("palette.TFrame", relief=[("focus", "solid")],
        ...
        bar = ttk.Frame(self, borderwidth=2, relief=tk.SOLID)
        self.bar = GradientBar(bar, hue=hue, width=200, highlightthickness=0)
        self.bar.pack()

        # --- ColorSquare
        square = ttk.Frame(self, borderwidth=2, relief=tk.SOLID)
        ...
        preview_frame = ttk.Frame(frame, relief=tk.SOLID, borderwidth=2)
        ...
        for i, col in enumerate(PALETTE):
            f = ttk.Frame(palette, borderwidth=1, relief=tk.SOLID,
                          style="palette.TFrame")
            l = tk.Label(f, background=col, width=2, height=1)
            l.bind("<1>", self._palette_cmd)
            f.bind("<FocusOut>", lambda e: e.widget.configure(relief=tk.SOLID))
            l.pack()
            f.grid(row=i % 2, column=i // 2, padx=2, pady=2)

        col_frame = ttk.Frame(self)
        # --- hsv
        hsv_frame = ttk.Frame(col_frame, relief=tk.SOLID, borderwidth=2)
        ...
        ttk.Label(hsv_frame, text=tr("Hue")).grid(row=0, column=0, sticky="e",
                                                 padx=4, pady=4)
        ttk.Label(hsv_frame, text=tr("Saturation")).grid(row=1, column=0, sticky="e",
                                                        padx=4, pady=4)
        ttk.Label(hsv_frame, text=tr("Value")).grid(row=2, column=0, sticky="e",
                                                   padx=4, pady=4)

        # --- rgb
        rgb_frame = ttk.Frame(col_frame, relief=tk.SOLID, borderwidth=2)
        ...
        s_red.delete(0, tk.END)
        s_red.insert(0, self._old_color[0])
        s_green.delete(0, tk.END)
        s_green.insert(0, self._old_color[1])
        s_blue.delete(0, tk.END)
        ...
        ttk.Label(rgb_frame, text=tr("Red")).grid(row=0, column=0, sticky="e",
                                                 padx=4, pady=4)
        ttk.Label(rgb_frame, text=tr("Green")).grid(row=1, column=0, sticky="e",
                                                   padx=4, pady=4)
        ttk.Label(rgb_frame, text=tr("Blue")).grid(row=2, column=0, sticky="e",
                                                  padx=4, pady=4)
        # --- hexa
        hexa_frame = ttk.Frame(col_frame)
        hexa_frame.pack(fill="x")
        self.hexa = ttk.Entry(hexa_frame, justify=tk.CENTER, width=10, name="entry")
        self.hexa.insert(0, old_color.upper())
        ttk.Label(hexa_frame, text="Hex").pack(side=tk.LEFT, padx=4, pady=(4, 1))
        self.hexa.pack(side=tk.LEFT, padx=6, pady=(4, 1), fill="x", expand=True)
        ...
        if alpha:
            ...
            alphabar = ttk.Frame(alpha_frame, borderwidth=2, relief=tk.SOLID)
            ...
            s_alpha.delete(0, tk.END)
            s_alpha.insert(0, self._old_alpha)
            alphabar.grid(row=0, column=0, padx=(0, 4), pady=4, sticky="w")
            ttk.Label(alpha_frame, text=tr("Alpha")).grid(row=0, column=1, sticky="e",
                                                         padx=4, pady=4)
        ...
        ttk.Button(button_frame, text=tr("Cancel"),
    ...
    def _select_all_spinbox(event):
        ...
        event.widget.selection("range", 0, tk.END)
        return "break"
    ...
    def _reset_preview(self, event):
        ...
        label.master.configure(relief=tk.SOLID)
        args = self._old_color
        if self.alpha_channel:
    ...
    def _update_color_hexa(self, event=None):
        ...
        if re.match(r"^#[0-9A-F]{6}$", color):
            ...
            if self.alpha_channel:
                a = self.alpha.get()
                self.hexa.insert(tk.END, ("%2.2x" % a).upper())
...
def askcolor(master=None, title=tr("Color Chooser"), color=(255, 0, 0), alpha=False):
    """
    Opens a :class: `ColorPicker` dialog and return the chosen color.
    ...
    :type alpha: bool
    """
    chooser = ColorPicker(master, title, color, alpha)
    chooser.wait_window(chooser)
    res = chooser.get_color()
    if res:
        return res[0], res[2]
    else:
        return None, None
