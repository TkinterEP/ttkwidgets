...
class FontFamilyEntryListbox(AutocompleteEntryListbox):
    """
    :class:`~ttkwidgets.AutocompleteEntryListbox` listing all font families available on the system with a Scrollbar on the right with the option
    ...
    def __init__(self, master=None, callback=None, **kwargs):
        ...
        :param kwargs: keyword arguments passed to :class:`~ttkwidgets.AutocompleteEntryListbox`, in turn passed to :class:`tk.Listbox`
        """
        kwargs['completevalues'] = sorted(set(font.families()))
        AutocompleteEntryListbox.__init__(self, master, **kwargs)
        self._callback = callback
        self.bind("<<ItemSelect>>", self._on_click)
